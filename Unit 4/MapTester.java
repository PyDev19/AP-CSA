//Name - Atharva Mishra
//Class - Patterson 3B
//Description - This class is used to run and test the MapIllustrator class to see if it functions as intended.
...
public class MapTester {
    ...
    public static void testDrawPath(MapIllustrator map, Graphics g) {
        ...
        g.setColor(Color.GREEN); // can set the color of the 'brush' before drawing, the method doesn't need to
                                 // worry about it

        int totalChange = map.drawPath(g, 200); // find greedy path for row 200

        System.out.println("Lowest-elevation-change path starting at row 200: " + totalChange); // should be
                                                                                                // ~18,000-19,000
    }

    /**
     * test the getIndexOfLowest method, which finds the overall best path /
     * starting row
     */
    public static void testBestPath(MapIllustrator map, Graphics g) {
        ...
        System.out.println(
                "Overall best lowest-elevation-change path at row: " + bestRow + ", total change of: " + totalChange);
    }
}